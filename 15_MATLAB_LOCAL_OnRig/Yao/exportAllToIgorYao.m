function [pnameOut]=exportAllToIgorYao(prefix, template, clearFromMemory, avgMode, forceLoadFromDisk, sizeLimit)
    ...
    tic
        ...
        for counter=1:length(files)	%loop through the filelist
            if ~files(counter).isdir	% made sure its a file and not a directory
             %  [path, fname, fext, ver]=fileparts(files(counter).name); % old version
                [path, fname, fext]=fileparts(files(counter).name); % new version (Yao revised on 5/17/2019)
        ...
        if ~any(strcmp(fname, doneFiles))	% was it exported already?
                if iswave(fname)
                    disp(['Exporting ' fname]);
                	if isempty(fext)
                    	fext='.itx';
                	end
                  
                    exportWave(fname, prefix, fullfile(pnameOut, [filePrefix fname fext]));
                    if clearFromMemory 
                        evalin('base', ['clear ' fname]);
                    end
                end	
