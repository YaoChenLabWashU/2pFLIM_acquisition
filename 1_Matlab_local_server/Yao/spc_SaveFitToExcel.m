...
for fc=FLIMchannels
    if state.analysisMode~=1
        ...
        spc_WriteToExcelCOM(0,0,CyclePosition);   %Write the cycle Position to Excel.
        % disp(['NextCol' num2str(spc.analysis.excelNextCol)]);
        spc.analysis.excelNextCol=spc.analysis.excelNextCol+str2num(CyclePosition)-1;
        % disp(['NextCol' num2str(spc.analysis.excelNextCol)]);
        rng=spc_WriteToExcelCOM(0,0,spc_calcEmpiricalMean(fc)); rng.NumberFormat='0.00';
    end
    % Below was added for debugging.
...
spc_SaveFitToWave %turn on if want to export to Igor files
end