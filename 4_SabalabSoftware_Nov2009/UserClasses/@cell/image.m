...
for counter = 1:length(in)
    ...
    if counter == 1
        ...
        ax=axes('Parent', f, 'Color', 'None', 'NextPlot', 'add', 'UserData', UserData, ...
            'XLimMode', 'auto', 'YLimMode', 'auto', 'Tag', ['Axis' num2str(length(allAx)+1)]);        figureName=[waveName];
        setWaveContextMenu(ax);    
    end
    UserData=[];
    UserData.name = {'' '' waveName} ;
    UserData.timeStamp = [ 0 0 wv.timeStamp];
    h=image('CData', wv.data, 'Parent', ax, 'UserData', UserData,'XData', makeXData(wv), 'YData', makeYData(wv), 'Tag', waveName);
...
rescaleAxis(ax,'yfraction',0, 'reorder', 1);
% Parse input parameter pairs and rewrite values.
counter=1;
