	...
	%		ivFlipSlice(state.imageViewer.objStructs(objectNumber).analysisSlice(timePoint));
			...
			for channelCode=[-1 -2]; % state.imageViewer.validAnaChannels
				...
				if state.imageViewer.analyzeBoxes
					...
					if channelCode==-1
						nPGreen=round(boxData/25);
						gMass=mass;
						nEst=zeros(1,10);
						for counter=1:10
							nPTestG=25*poissrnd(nPGreen);
							image(nPTestG);
							drawnow
							[xcG, ycG, massG, peakValG, pixSizeG, lineParamG]=ivCenterOfMass(nPTestG);
							nEst(counter)=sqrt((xc-xcG)^2+(yc-ycG)^2);
						end
						mean(nEst)
					else
						nPhotons=round(boxData/25);
						nPGreen=round(massG/mass*boxData/25);
						nEst=zeros(1,10);
						for counter=1:10
							nPTestG=25*poissrnd(nPGreen);
							[xcG, ycG, massG, peakValG, pixSizeG, lineParamG]=ivCenterOfMass(nPTestG);
							nPTestR=25*poissrnd(nPhotons);
							[xcR, ycR, massR, peakValR, pixSizeR, lineParamR]=ivCenterOfMass(nPTestR);
							nEst(counter)=sqrt((xcR-xcG)^2+(ycR-ycG)^2);
						end
						mean(nEst)
