function exportAvg(waveList, prefix, outputFile)
	...
	for avgName=waveList
		...
		for name=[avgName avgComponentList(avgName{1})];
			if iswave(name{1})
				if any(strcmp(name{1}, doneFiles))	% was it exported already?
					% yes
					disp([name{1} ' already exported.  Skipping reexport']);
				else
					%no, then export and add to list
