...
for nr=1:nROI
    ...
    for fc=FLIMchannels
        ...
        try  % Projection from spc (mean)
            ...
            set(gui.spc.projects{fc}.(['countROI' num2str(nr,1)]),'String',num2str(val,3));
            set(gui.spc.projects{fc}.(['countROI' num2str(nr,1)]),'BackgroundColor','k');
        catch
...
%             val=(sum(maskedLife.*tt(:))/sum(maskedLife))*spc.datainfo.psPerUnit/1000-t_Offset;
        ...
        set(gui.spc.lifetimeMaps{fc}.(['lifeROI' num2str(nr,1)]),'String',num2str(val,3));
        set(gui.spc.lifetimeMaps{fc}.(['lifeROI' num2str(nr,1)]),'BackgroundColor','k');
    end  % fc=FLIMchannels
...
function val=calcVal(img,mask,bkg)
val=sum(sum((img-bkg).*mask))/sum(sum(mask));