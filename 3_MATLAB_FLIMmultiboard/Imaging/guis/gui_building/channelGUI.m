...
% Last Modified by GUIDE v2.5 23-Nov-2011 09:12:39
...
% --- Executes on button press in acquiringChannel5.
function acquiringChannelN_Callback(h, eventdata, handles)
% Stub for Callback of the uicontrol handles.savingChannel2.
% updates all the checkboxes accross a row when you acquire one channel.
	global state
	state.internal.channelChanged=1;
	genericCallback(h);
    tObj=get(h,'Tag');  % gy changed to be more general
    chan=tObj(end);
    val=get(h,'Value');
    state.acq.(['savingChannel' chan]) = val;
    updateGUIByGlobal(['state.acq.savingChannel' chan]);
    state.acq.(['imagingChannel' chan]) = val;
    updateGUIByGlobal(['state.acq.imagingChannel' chan]);
    state.acq.(['maxImage' chan]) = val;
    updateGUIByGlobal(['state.acq.maxImage' chan]);
