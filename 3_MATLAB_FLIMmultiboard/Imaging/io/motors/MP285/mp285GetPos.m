...
if ~mp285Error
	try
		% gy changed following line from 'long' to 'int32'
        array = fread(state.motor.serialPortHandle, 3, 'int32');		% read position information (12bytes) including CR (1 byte)
	catch
		mp285Error=1;
	end
end
...
if ~mp285Error
	% gy:
    try % do a flush, without the state variable resets
...
if length(array)<3 %gy | length(dummy)<1				% check if data is avaiable
