...
% Februarry 1, 2001
    ...
    if state.acq.dualLaserMode==1   % lasers go simulataneously therefor one image window per laser
	    channelList=1:state.init.maximumNumberOfChannelsSaved; % CHANGED gy 201111
    else
        channelList=[1:state.init.maximumNumberOfChannelsSaved 11:10+state.init.maximumNumberOfChannelsSaved]; % CHANGED gy 201111
    end
