...
% Laser Parking Mirror Data Output Acquisition
	...
	% PCell output board for FOCUS
		...
		% GY 08/18/10 Added for clock control of spc board
		if state.acq.createLineClock 
			addchannel(pcellFocusOutput, state.acq.lineClockIndex);
			addchannel(pcellFocusOutput, state.acq.frameClockIndex);
		end
	...
	% PCell output board for GRAB
		...
		% GY 08/18/10 Added for clock control of spc board
		if state.acq.createLineClock 
			addchannel(pcellGrabOutput, state.acq.lineClockIndex);
			addchannel(pcellGrabOutput, state.acq.frameClockIndex);
		end
